...
export const all = async (req: Request, res: Response) => {
    const newsQuerySnapshot = await db.collection('customers').get();
...
export const one = async (req: Request, res: Response) => {
    try {
        const newOne = await db.collection('customers').doc(req.params.id).get();
...
export const add = async (req: Request, res: Response) => {
    try {
        const newDoc = await db.collection('customers').add(req.body);
...
export const update = async (req: Request, res: Response) => {
    try {
        await db.collection('customers').doc(req.params.id).set(req.body, { merge: true })
